"""3rd party Windows-specific stdio utilities from https://github.com/modelcontextprotocol/python-sdk/blob/main/src/mcp/os/win32/utilities.py"""
...
def _create_job_object() -> object | None:
    """Creates a Windows Job Object configured to terminate all its processes
    when closed."""
    ...
    try:
        ...
        logger.warning(
            "Failed to create Job Object for process tree management", exc_info=True
        )
...
def _maybe_assign_process_to_job(
    process: Process | FallbackProcess, job: object | None
) -> None:
    ...
    def _raise_if_no_handle(handle: object | None) -> object:
        if handle is None:
    ...
    try:
        ...
        if not process_handle:
            _raise_if_no_handle(process_handle)
        ...
        logger.warning(
            "Failed to assign process %d to Job Object", process.pid, exc_info=True
        )
        _close_job_handle(job)
