...
VALIDATION_FILE = """// Code generated by generategomodels; DO NOT EDIT.
...
def go_ident(value: str, *, exported: bool = True) -> str:
    ...
    ident = "".join(part[:1].upper() + part[1:] for part in cleaned)
...
def ref_to_schema_key(ref: str, *, major: int) -> str | None:
    prefix = "https://kiln.cyborgdev.cloud/schemas/"
    suffix = ".schema.json"

    if not ref.startswith(prefix) or not ref.endswith(suffix):
        return None

    relative = ref[len(prefix) : -len(suffix)]
...
def go_type_for_schema(
    ...
    *,
    ...
    if isinstance(ref, str):
        schema_key = ref_to_schema_key(ref, major=major)
        if schema_key is None:
            return "any"

        import_path = package_import_for_key(module_root, schema_key)
        alias = import_alias_for_key(schema_key)
        imports[alias] = import_path

        return f"{alias}.{type_name_for_key(schema_key)}"
...
def validate_tags_for_schema(schema: dict[str, Any], *, required: bool) -> list[str]:
    ...
    if schema_type == "string" and fmt == "uri":
        ...
        tags.append("uuid")
    elif schema_type == "string" and fmt == "date-time":
        # time.Time is already the generated type.
        pass
    ...
    *,
    ...
    for field in fields:
        ...
        if isinstance(enum_values, list) and enum_values:
            emit_enum(
                enum_type_name(parent_type_name, field.json_name),
                enum_values,
            )
...
def render_model_file(
    ...
    *,
    ...
    imports: dict[str, str] = {}
    imports["validator"] = "github.com/go-playground/validator/v10"
    ...
    chunks.append("\treturn validate.Struct(value)\n")
    chunks.append("}\n\n")
    chunks.append("var validate = validator.New()\n")
...
def write_root_package(output_root: Path, *, dry_run: bool) -> None:
    if dry_run:
        print(f"would write {output_root / 'validate.go'}")
        return

    output_root.mkdir(parents=True, exist_ok=True)
    (output_root / "validate.go").write_text(VALIDATION_FILE, encoding="utf-8")
...
def main() -> int:
    ...
    try:
        ...
        write_root_package(output_root, dry_run=args.dry_run)
