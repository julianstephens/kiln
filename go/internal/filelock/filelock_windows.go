...
import (
	...
	"github.com/julianstephens/kiln/go/internal/filelock/sysdll"
)
...
func lock(f File, lt lockType) error {
	...
	err := LockFileEx(Handle(f.Fd()), uint32(lt), reserved, allBytes, allBytes, ol)
	if err != nil {
		return &fs.PathError{
			Op:   lt.String(),
			Path: f.Name(),
...
func LockFileEx(file Handle, flags uint32, reserved uint32, bytesLow uint32, bytesHigh uint32, overlapped *syscall.Overlapped) (err error) {
